...
class ListHabitsToolTest {
    ...
    void execute_delega_en_getHabits_y_devuelve_la_lista_tal_cual() {
        List<HabitResponse> habits = List.of(
                new HabitResponse(UUID.randomUUID(), "Meditar", null, 3, 5, true, Instant.now(), Instant.now()),
                new HabitResponse(UUID.randomUUID(), "Ejercicio", null, 0, 2, false, Instant.now(), Instant.now()));

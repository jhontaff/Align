...
# Habit domain (`habit`) — REST complete, AI tools pending

Entity, DTOs, repository, mapper, service, controller, and tests are implemented; AI tools are not — this is the current state, so a new session can tell what's done versus what's still open:
...
- Done: `HabitMapper.toResponse(Habit habit, int streak)` maps from **two** source parameters, not one — `currentStreak` isn't a column on `Habit`, it's computed by the service and passed in explicitly (`@Mapping(target = "currentStreak", source = "streak")`). First mapper in the project that needs MapStruct's multi-source-parameter support, since Task/Transaction map 1:1 from a single entity.
...
- Done: test coverage — `HabitServiceImplTest` (16 tests: CRUD + not-found paths, `completeHabit` idempotency, and dedicated streak edge cases — zero with no completions, consecutive-day counting, the grace-day rule, reset after a gap, and a broken streak not resuming past a gap) and `HabitControllerTest` (6 tests: delegation and status code per endpoint). Same style as `TransactionServiceImplTest`/`TransactionControllerTest` — plain JUnit 5 + Mockito + AssertJ, no Spring context. Streak tests use dates relative to `LocalDate.now()` rather than fixed dates, since the behavior under test depends on what day it is.

Next step for this domain: AI tools (`create_habit`, `list_habits`, `complete_habit`), following the same [Tool argument parsing](#tool-argument-parsing-aitool) conventions as Task/Finance, and updating `SystemPromptBuilder` again once they exist.

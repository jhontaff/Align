...
- **10 keys, each from a separate Google Cloud project** — confirmed against Google's own docs (2026-09-04, `ai.google.dev/gemini-api/docs/rate-limits`: "Rate limits are applied per project, not per API key"), correcting an earlier, less precise version of this decision that said "10 separate Google accounts." The boundary is the Cloud project, not the account or the key — keys sharing a project share its quota, so pooling only works if each key lives in its own project. A single Google account can own multiple projects, so this doesn't strictly require 10 separate email accounts. Not verified: whether Google caps how many free-tier-eligible projects one account can create — recommended check before generating all 10: create 2 keys in 2 projects under the same account, deliberately exhaust the rate limit on one, confirm the other still responds.
